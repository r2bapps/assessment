...
class UserListViewModel @Inject constructor(
        ...
        if (!isLastPage) {
            getUsersUseCase.execute(page)
                ...
                .onEach {
                    when (it) {
                        is Either.Failure -> {
                            _event.send(Event.Error(it.error))
                        }
                        is Either.Success -> {
                            checkLastPage(it.data)
                            _uiState.value = UserUiState.Load(it.data.toViewModel())
                        }
                    }
                }
                .onCompletion {
                ...
                .catch {
                    isLoadingMoreEnabled = true
                    _event.send(Event.GlobalError(it))
                }
            ...
            getUsersUseCase.execute(page)
                ...
                .onEach {
                    when (it) {
                        ...
                        is Either.Success -> {
                            checkLastPage(it.data)
                            _uiState.value = UserUiState.LoadMore(it.data.toViewModel())
                        }
                    }
                }
    ...
    fun removeUser(position: Int, user: User) {
        removeUserUseCase.execute(user.id)
            ...
            .catch { _event.send(Event.GlobalError(it)) }
            .launchIn(viewModelScope)
    }
    ...
    sealed class Event {
        ...
        data class GlobalError(val error: Throwable): Event()
        data class Error(val error: DomainError): Event()
    }
